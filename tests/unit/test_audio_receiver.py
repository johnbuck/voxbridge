...
@pytest.mark.unit
def test_audio_receiver_initialization(event_loop):
    ...
    assert receiver.loop == mock_loop
...
@pytest.mark.unit
def test_write_creates_user_buffer_on_first_audio(event_loop):
    ...
    receiver = AudioReceiver(mock_vc, mock_speaker_mgr, event_loop)
    ...
    # Create mock audio data (VoiceData object with opus attribute)
    audio_data = MagicMock()
    audio_data.opus = b'\x00' * 960

    with patch('asyncio.run_coroutine_threadsafe') as mock_run_coro:
        mock_run_coro.return_value = MagicMock()  # Mock Future
        receiver.write(mock_user, audio_data)
        ...
        # Verify run_coroutine_threadsafe was called (for on_speaking_start)
        assert mock_run_coro.call_count >= 1


@pytest.mark.unit
def test_write_queues_opus_packet_from_dict(event_loop):
...
@pytest.mark.unit
def test_write_queues_opus_packet_direct(event_loop):
    ...
    # Send audio data as VoiceData mock
    audio_data = MagicMock()
    audio_data.opus = b'\xFF' * 960
...
@pytest.mark.unit
def test_write_ignores_none_user(event_loop):
    ...
    receiver.write(None, audio_data)
...
@pytest.mark.unit
def test_write_ignores_empty_packet(event_loop):
    ...
    # Empty VoiceData (no opus)
    audio_data = MagicMock()
    audio_data.opus = None
...
@pytest.mark.unit
def test_write_handles_queue_full(event_loop):
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_audio_stream_generator_yields_chunks(event_loop):
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_audio_stream_generator_stops_on_sentinel(event_loop):
...
@pytest.mark.unit
def test_speaker_manager_integration_called(event_loop):
    ...
    with patch('asyncio.run_coroutine_threadsafe', return_value=mock_future) as mock_run_coro:
        receiver.write(mock_user, audio_data)

        # Verify run_coroutine_threadsafe was called
        assert mock_run_coro.called
        assert '88888' in receiver.user_tasks
        assert receiver.user_tasks['88888'] == mock_future
...
@pytest.mark.unit
def test_cleanup_sends_sentinel_values(event_loop):
...
@pytest.mark.unit
def test_cleanup_cancels_user_tasks(event_loop):
...
@pytest.mark.unit
def test_cleanup_clears_all_data(event_loop):
...
@pytest.mark.unit
def test_multiple_users_concurrent(event_loop):
    ...
    with patch('asyncio.create_task'):
        # Create audio data for each user
        audio1 = MagicMock()
        audio1.opus = b'\x01' * 960
        audio2 = MagicMock()
        audio2.opus = b'\x02' * 960
        audio3 = MagicMock()
        audio3.opus = b'\x03' * 960

        # All users send audio
        receiver.write(user1, audio1)
        receiver.write(user2, audio2)
        receiver.write(user3, audio3)
...
@pytest.mark.unit
def test_same_user_multiple_packets(event_loop):
